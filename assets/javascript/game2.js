var hangMan = {
 ...
 //---------Object of Helper Functions for Game ------------
    ...
    "initSearchBankArray" : function(aString) {
      var charArray = [];
      for(var i = 0; i < aString.length; i++) {
           var iChar = aString[i].toLowerCase();
           if (!(charArray.includes(iChar)) && (iChar != " ")) {
            charArray[i].push(iChar);
            } else if (iChar = " ") {
              spaces++;;
            }
        } return charArray;
...
/*
    "initSearchBankObject": function(aString) {
        var obj = {"trueChars" : 0};
        for(var i = 0; i < aString.length; i++) {
           var iChar = aString[i].toLowerCase();
...
/* **************** end of helpers ************** */
 ...
 "checkWord2": function() {
  ...
  "checkWord2": function() {
    return (this.searchBank.indexOf(this.userGuess) === -1) ? (this.charsCorrect++, true) : (this.failChances--, false);
  },
  ...
  "getGuess": function(event) {
    this.userGuess = event.target.getAttribute("value");
  },
  ...
  "displayPlaceholders": function()
  {
      ...
      for (var x = 0; x < (this.theWord).length; x++)
      {
        ...
        newS.setAttribute("class", this.searchBank.indexOf(x));
        //console.log((this.searchBank[(this.theWord[x])[0]]));
        //newS.setAttribute("class",(this.searchBank[(this.theWord[x])[0]]));
        /*if (this.searchBank[this.theWord[x]].property != " ") //this conditional doesn't work bcz you can't access the property name.
        { */
          if (this.theWord[x] != " "){
...
"displayLetters": function() {
    for (var i=0; i< (this.searchBank).length; i++) {
      var spaces = document.getElementById("placeHolder");
      if (spaces.getAttribute("class") == (this.searchBank.indexOf(this.userGuess))) {
